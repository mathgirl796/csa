...
/* 测试：fasta文件计数测试 */
void testCountFasta() {
    long* counts = CountFasta("data/test.fna");
    printf("num reads: %ld\n", counts[0]);
    for (int i = 1; i <= counts[0]; ++i) {
        printf("read %d length: %ld\n", i, counts[i]);
    }
    free(counts);
}
...
/* 测试：快速排序10个整数 */
static long A[10] = {5,7,3,8,6,2,4,0,9,1};
static int cmp(long l, long r) {
    if (A[l] < A[r]) return -1;
...
static char* compressedCmp;
static long compressedCmpLen;
...
void testQuickNC_008253_fna() {
    ...
    char* NC_008253FilePath = "data/NC_008253.fna";
...
static int bsArray[10] = {0, 2, 4, 4, 4, 6, 7, 9, 9, 9};
static int BSIntCmpFunc(const void* target, long pos) {
    int t = *(int*)target;
